...
C_DEEP_BLUE = RGBColor(30, 60, 114)      # #1E3C72 (Deep Slate Blue)
...
C_DARK_CARD_BORDER = RGBColor(51, 65, 85) # #334155 (Cyber Dark Border)
...
def add_card(slide, left, top, width, height, title, items, 
             ...
             accent_color=None, title_color=C_DARK_NAVY, font_size=11, title_size=13):
    ...
    if accent_color:
        strip = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, int(Pt(6)))
        strip.fill.solid()
    ...
    tf.margin_left = int(Pt(14))
    tf.margin_right = int(Pt(14))
    tf.margin_top = int(Pt(12))
    tf.margin_bottom = int(Pt(12))
    ...
    for item in items:
        ...
        if ":" in item and len(item.split(":", 1)[0]) < 38:
...
def add_badge(slide, left, top, width, height, text, bg_color=C_SIH_BLUE, text_color=C_WHITE, font_size=10):
...
print("deck_builder module loaded successfully.")
C_SLATE_BLUE = RGBColor(30, 58, 95)
